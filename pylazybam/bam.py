...
def get_bin(alignment):
    return struct.unpack('<H', alignment[14:16])[0]

def get_number_cigar_opperations(alignment):
...
def get_pair_ref_index(alignment):
    return struct.unpack('<i', alignment[24:28])[0]

def get_pair_pos(alignment):
    return struct.unpack('<i', alignment[28:32])[0]

def get_template_len(alignment):
...
def decode_base_qual(raw_base_qual, offset=33):
    return ''.join([chr(q + offset) for q in list(raw_base_qual)])

def is_flag(alignment, flag):
